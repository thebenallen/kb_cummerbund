...
#                  'description': "Dendrogram including replicates based on the JS (Jensen-Shannon divergence) distance" },
    ...
    def create_heatmap_de_genes(self, ctx, heatmapParams):
        # ctx is the context object
        # return variables are: returnVal
        ...
        rparams['plotscript'] = join(system_params['rscripts'], "heatmapplot.R")
        rparams['include_replicates'] = 0

        #Command line for without replicates
        roptstr_basic_heatmap = script_util2.get_command_line_heatmap_basic (rparams)


        #Command line for with replicates
        ...
        # List of plots to generate
        plotlist = [
                { 'roptstr': roptstr_basic_heatmap,
                  'title': "Heatmap",
                  'description': "Heatmap", 
                  'exp' : fparams['ws_expression_matrix_id1']
                  },
                  
                { 'roptstr': roptstr_basic_heatmap_rep,
                  'title': "Heatmap",
                  'description': "Heatmap", 
                  'exp' : fparams['ws_expression_matrix_id2']
        ...
        # Populate the output object
        outputobject['cummerbundplotSet'] = fparams['cummerbundplotset']
        ...
        res = ws_client.save_objects({
            ...
            "objects": [{
                "type":"KBaseRNASeq.cummerbund_output",
                "data":outputobject,
                "name":fparams["ws_cummerbund_output"]}]
        })



  #      scriptfile = "tmpheatmap"
  #      outjson = script_util2.upload_feature_value (system_params, fparams)
...
#                  eo = json.load(et)
#        eo['type']='untransformed'
#        self.__LOGGER.info(workspace + self.__SCRATCH + outjson + fparams['ws_expression_matrix_id'])
#        ws_client.save_objects({'workspace' : workspace,
#            'objects' : [{ 'type' : 'KBaseFeatureValues.ExpressionMatrix',
#                           'data' : eo,
#                           'name' : fparams['ws_expression_matrix_id']
#                        }]})



        #END create_heatmap_de_genes

        # At some point might do deeper type checking...
        if not isinstance(returnVal, basestring):
            raise ValueError('Method create_heatmap_de_genes return value ' +
                             'returnVal is not type basestring as required.')
        # return the results
        return [returnVal]

    def create_volcano_plot(self, ctx, volcanoplotParams):
        # ctx is the context object
        # return variables are: returnVal
        #BEGIN create_volcano_plot
        #END create_volcano_plot

        # At some point might do deeper type checking...
        if not isinstance(returnVal, basestring):
            raise ValueError('Method create_volcano_plot return value ' +
                             'returnVal is not type basestring as required.')
        # return the results
        return [returnVal]
