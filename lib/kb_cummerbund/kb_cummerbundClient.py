...
class kb_cummerbund(object):
    ...
    def generate_cummerbund_plots_check(self, job_id):
        resp = self._call('kb_cummerbund.generate_cummerbund_plots_check', [job_id])
        return resp[0]

    def generate_cummerbund_plots(self, cummerbundParams, json_rpc_context = None):
        job_id = self.generate_cummerbund_plots_async(cummerbundParams, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.generate_cummerbund_plots_check(job_id)
    ...
    def create_expression_matrix_check(self, job_id):
        resp = self._call('kb_cummerbund.create_expression_matrix_check', [job_id])
        return resp[0]

    def create_expression_matrix(self, expressionMatrixParams, json_rpc_context = None):
        job_id = self.create_expression_matrix_async(expressionMatrixParams, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.create_expression_matrix_check(job_id)
            if job_state['finished']:
                return job_state['result'][0]
    ...
    def create_interactive_heatmap_de_genes_check(self, job_id):
        resp = self._call('kb_cummerbund.create_interactive_heatmap_de_genes_check', [job_id])
        return resp[0]

    def create_interactive_heatmap_de_genes(self, interactiveHeatmapParams, json_rpc_context = None):
        job_id = self.create_interactive_heatmap_de_genes_async(interactiveHeatmapParams, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.create_interactive_heatmap_de_genes_check(job_id)
            if job_state['finished']:
                return job_state['result'][0]
