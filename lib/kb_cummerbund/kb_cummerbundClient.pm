...
=head2 create_heatmap_de_genes

  $return = $obj->create_heatmap_de_genes($heatmapParams)
...
sub create_heatmap_de_genes
{
    my($self, @args) = @_;
    my $job_id = $self->create_heatmap_de_genes_async(@args);
    while (1) {
        Time::HiRes::sleep($self->{async_job_check_time});
        my $job_state_ref = $self->create_heatmap_de_genes_check($job_id);
        if ($job_state_ref->{"finished"} != 0) {
            if (!exists $job_state_ref->{"result"}) {
                $job_state_ref->{"result"} = [];
            }
...
sub create_heatmap_de_genes_async {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function create_heatmap_de_genes_async (received $n, expecting 1)");
    }
    {
        ...
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to create_heatmap_de_genes_async:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => 'create_heatmap_de_genes_async');
        }
    }
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "kb_cummerbund.create_heatmap_de_genes_async",
        params => \@args});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method create_heatmap_de_genes_async",
                        status_line => $self->{client}->status_line,
                        method_name => 'create_heatmap_de_genes_async');
    }
}

sub create_heatmap_de_genes_check {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function create_heatmap_de_genes_check (received $n, expecting 1)");
    }
    {
        my($job_id) = @args;
        my @_bad_arguments;
        (!ref($job_id)) or push(@_bad_arguments, "Invalid type for argument 0 \"job_id\" (it should be a string)");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to create_heatmap_de_genes_check:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => 'create_heatmap_de_genes_check');
        }
    }
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "kb_cummerbund.create_heatmap_de_genes_check",
        params => \@args});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method create_heatmap_de_genes_check",
                        status_line => $self->{client}->status_line,
                        method_name => 'create_heatmap_de_genes_check');
    }
}
